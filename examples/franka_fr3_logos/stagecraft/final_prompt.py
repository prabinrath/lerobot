...
prompt_template = textwrap.dedent("""
            ...
            You are provided with {n_videos} spatiotemporal-mosaics. Each mosaic consists of equally-spaced "front_camera_RGB, wrist_camera_RGB" frames showing the progression of a policy rollout.
            The mosaics are labeled v1 through v{n_videos}. Additionally, you are provided with the policy rollout results that are either "SUCCESS" or "FAILURE" tags. The mosaic labels and the result tags are as follows:
            ...
            In order to gauge the performance of the policy rollouts, you need to understand the following prerequisites:
                    ...
                    3. @action(object) :  This is a predefined primitive action that the robot can take to change the environment category of the workspace.
                                          List of primitives available with the robot are: [@remove, @add]
            ...
            ****ALGORITHM****
            1. Identify the [object]s present in the robot's workspace across all the {n_videos} spatiotemporal-mosaics. 
               ...
               Then build the corresponding environment category for each mosaic. NOTE: Multiple mosaics might have the same environment category if they have the same set of [object]s.
                                             
            2. Identify the unique environment categories and assign success rates to each of them based on the result tags associated with the spatiotemporal-mosaics.
                    For example: If the environment category < coffee_cup, water_bottle > appears 5 times in the spatiotemporal-mosaics and the result tags for this environment category are
            ...
            3. Identify the environment categories with high success rates. Out of the environment categories with high success rates, deduce the environment category with the minimum number of objects.
         
            4. Consider the "new_initial_state_image" provided at the end of the spatio-temporal mosaic sequence. This is the initial state image of a new robot workspace.
                ...
                environment categories previously identified from the {n_videos} spatio-temporal mosaics, and no performance tag will be available for the "new_initial_state_image".
            ...
            Field descriptions:
            - "object_list": List of objects present in the workspace across all the spatiotemporal-mosaics and the "new_initial_state_image".
            - "unique_environment_categories": The list of unique environment categories across all the spatiotemporal-mosaics.
