...
class SAM_VLM_Planner:
    ...
    def send_to_vlm(self, results, task_instruction, new_initial_state,
                    prompt_template=None, model_id="gpt-5.2-pro",
        ...
        for i, (video_key, data) in enumerate(results.items(), 1):
            ...
            contents.append({"type": "input_text", "text": f"{label} (tag: {data['tag'].upper()}):"})
            contents.append({"type": "input_image", "image_url": f"data:image/png;base64,{image_b64}"})
        ...
        if isinstance(new_initial_state, np.ndarray):
            ...
            current_obs_b64 = base64.b64encode(cur_obs_buf.tobytes()).decode("utf-8")
        else:
            current_obs_buffer = io.BytesIO()
            new_initial_state.save(current_obs_buffer, format='PNG')
            current_obs_b64 = base64.b64encode(current_obs_buffer.getvalue()).decode("utf-8")
        contents.append({"type": "input_text", "text": "new_initial_state_image (predict for this environment):"})
        contents.append({"type": "input_image", "image_url": f"data:image/png;base64,{current_obs_b64}"})
